...
public class DuelfField_CardDetailViewer : MonoBehaviour
{
    ...
    public void SetCardListToBeDisplayed(ref List<Card> carditemList, int index, DuelField_HandClick.ClickAction clickAction)
    {
    ...
    private async Task UpdateDisplayAsync(DuelField_HandClick.ClickAction clickAction)
    {
        Card thisCard = this._carditemList[_currentIndex];
        ...
        if (thisCard != null)
        {
            ...
            CurrentDisplayingCard.Init(thisCard.ToCardData());
            ...
            if (clickAction.Equals(DuelField_HandClick.ClickAction.ViewAndUseArt))
            {
                ...
                //for each art that the current card(thisCard) have, we need to instantiate a new card
                foreach (Art currentArt in thisCard.Arts)
                {
                    if (currentArt.Name.Equals("Retreat") && !thisCard.curZone.Equals(Lib.GameZone.Stage))
                        continue;
                    ...
                    DuelAction duelaction = new() { playerID = DuelField.INSTANCE.playersType[PlayerInfo.INSTANCE.PlayerID] };
                    duelaction.used = thisCard.ToCardData();
                        ...
                        && thisCard.PassSpecialDeclareAttackCondition(currentArt))
                    {
                        if (currentArt.Name.Equals("Retreat"))
                            ...
                            {
                                CloseDisplayed();
                                duelaction.selectedSkill = itemButton.transform.Find("ArtButton").Find("Name").GetComponent<TMP_Text>().text;
            ...
            if (clickAction.Equals(DuelField_HandClick.ClickAction.ViewAndUseOshiBothSkills) || clickAction.Equals(DuelField_HandClick.ClickAction.ViewAndUseSPOshiSkill) || clickAction.Equals(DuelField_HandClick.ClickAction.ViewAndUseOshiBothSkills))
            {
                ...
                string translatedOshiSkill = await GoogleTranslateAPI.TranslateTextHandle(thisCard.oshiSkill);
                string translatedSpOshiSkill = await GoogleTranslateAPI.TranslateTextHandle(thisCard.spOshiSkill);
                ...
                DuelAction duelaction = new() { used = thisCard.ToCardData() };
                duelaction.used.curZone = thisCard.curZone;
            ...
            if (clickAction.Equals(DuelField_HandClick.ClickAction.OnlyView))
            {
                ...
                thisCard.Init(thisCard.ToCardData());

                //need to make this better later, get each card attack and match with its text effect
                string translatedArts = await GoogleTranslateAPI.TranslateTextHandle(thisCard.artEffect.Replace(";", "\n"));
                string translatedOshiSkill = await GoogleTranslateAPI.TranslateTextHandle(thisCard.oshiSkill);
                string translatedSpOshiSkill = await GoogleTranslateAPI.TranslateTextHandle(thisCard.spOshiSkill);
                string translatedAbilityText = await GoogleTranslateAPI.TranslateTextHandle(thisCard.abilityText);
                ...
                textComponent.text = RemoveEmptyLines(
                    ...
                    $"{(thisCard.cardNumber != null ? "\n" + thisCard.cardNumber : string.Empty)}"
                    );
            }
        }
    }
